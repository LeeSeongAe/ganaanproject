...
//    var selectedImage: [IndexPath]
    ...
    var photosDataSource: PhotosCollectionViewDataSource?
    var albumsDataSource: AlbumTableViewDataSource
    let cameraDataSource: CameraCollectionViewDataSource
...
//        self.selectedImage = selectedImage
    ...
    @objc func collectionViewLongPressed(_ sender: UIGestureRecognizer) {
        if sender.state == .began {
            ...
            if let vc = previewViewContoller, let indexPath = indexPath, let cell = collectionView?.cellForItem(at: indexPath) as? BsPhotoCell, let asset = cell.asset {
    ...
    func initializePhotosDataSourceWithFetchResult(_ fetchResult: PHFetchResult<PHAsset>) {
        let newDataSource = PhotosCollectionViewDataSource(fetchResult: fetchResult, assetStore: assetStore, settings: settings)
